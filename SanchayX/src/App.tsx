    ...
    <div className={`w-full flex-1 transition-all duration-300 ${pagePatternClass}`}>
      <main className="w-full max-w-[1750px] mx-auto min-h-[calc(100vh-140px)] relative px-3 lg:px-5 py-3.5">
        <Suspense fallback={<PageLoaderFallback />}>
          {/* If a dedicated sub-tab option is selected, render dedicated Trading Console Page */}
          {activeSubTab ? (
            <TradingConsolePage />
          ) : (
            <>
      ...
      <footer className="border-t border-[var(--border-color)] bg-[var(--bg-card)] px-6 py-4 mt-8 text-xs text-[var(--text-secondary)] w-full">
        <div className="max-w-[1750px] mx-auto flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-[var(--text-primary)]">SanchayX Wealth OS</span>
            <span>— Semi-Automated Private Wealth Operating System</span>
          </div>
          <div className="flex items-center gap-4 text-[var(--text-muted)]">
            <span className="flex items-center gap-1">
              <ShieldCheck className="w-3.5 h-3.5 text-[var(--accent-emerald)]" /> Valuation Sentinel & Legal Tax Loopholes
            </span>
            <span>•</span>
            <span>Top 1000 Indian Equities (NSE/BSE) • 7.18% G-Sec • SGB Gold • Section 112A/70/71</span>
            <span>•</span>
            <span className="font-mono">v3.0 Wealth Edition</span>
          </div>
        </div>
      </footer>
    </div>
