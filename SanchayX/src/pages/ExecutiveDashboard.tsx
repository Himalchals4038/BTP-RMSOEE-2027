    ...
    <div className="p-4 lg:p-6 space-y-6 w-full">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 w-full">
        ...
        <div className="glass-card p-5 border-l-4 border-l-[var(--icici-orange)] flex flex-col justify-between">
          <div className="flex items-center justify-between text-[var(--text-secondary)] text-xs font-semibold">
            <span>DYNAMIC TOTAL NET WORTH ({currency})</span>
          ...
          <div className="my-2">
            ...
            <div className={`text-xs font-semibold flex items-center gap-1 mt-1 ${total24hPnl >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}>
          ...
          <div className="text-[10px] text-[var(--text-muted)] flex items-center justify-between">
            <span>Live Reactive Simulation Store</span>
            <span className="text-emerald-600 dark:text-emerald-400 font-bold">● Active Ledger</span>
          </div>
        </div>
        ...
        <div className="glass-card p-5 border-l-4 border-l-amber-500 flex flex-col justify-between">
          ...
          <div className="my-2">
            <div className="text-xl font-bold font-mono text-amber-600 dark:text-amber-400">
              {dynamicVaR95}% <span className="text-xs text-[var(--text-muted)] font-normal">(95% Conf.)</span>
            </div>
            <div className="text-xs text-[var(--text-secondary)] font-mono mt-0.5">
              99% VaR: <span className="text-rose-600 dark:text-rose-400 font-bold">{Number((dynamicVaR95 * 1.45).toFixed(1))}%</span>
            </div>
          </div>
          <div className="text-[10px] text-[var(--text-muted)]">MPT Parametric 1-Yr Horizon</div>
        </div>
        ...
        <div className="glass-card p-5 border-l-4 border-l-[var(--icici-red)] flex flex-col justify-between">
          ...
          <div className="text-[10px] text-[var(--text-secondary)]">
            Max Drawdown: <span className="text-rose-600 dark:text-rose-400 font-bold font-mono">-{kri.maxDrawdown}%</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="glass-card p-4 flex items-center justify-between">
          <div className="space-y-1">
            <span className="text-[10px] font-bold text-[var(--text-muted)] uppercase flex items-center gap-1.5">
              <Building2 className="w-3.5 h-3.5 text-blue-500" /> Demat Holdings Statement
            </span>
            <div className="text-lg font-mono font-black text-[var(--text-primary)]">
              {formatCompactCurrency(totalHoldingsValue, currency)}
            </div>
            <div className={`text-[10px] font-bold font-mono ${totalUnrealizedPnl >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}>
              {totalUnrealizedPnl >= 0 ? '+' : ''}{formatCompactCurrency(totalUnrealizedPnl, currency)} Unrealized
            </div>
          </div>
          <span className="text-xs font-mono font-bold px-2 py-1 rounded bg-blue-500/15 text-blue-500 border border-blue-500/20">
        ...
        <div className="glass-card p-4 flex items-center justify-between">
          <div className="space-y-1">
            <span className="text-[10px] font-bold text-[var(--text-muted)] uppercase flex items-center gap-1.5">
              <CreditCard className="w-3.5 h-3.5 text-emerald-500" /> Available Trading Margin
            </span>
            ...
            <div className="text-[10px] text-[var(--text-muted)]">
              Used MIS/F&O: <span className="font-mono font-bold text-amber-500">{formatCompactCurrency(wallet.usedMargin, currency)}</span>
            </div>
          </div>
          <span className="text-xs font-mono font-bold px-2 py-1 rounded bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20">
            Free Cash
          </span>
        </div>
        ...
        <div className="glass-card p-4 flex items-center justify-between">
          <div className="space-y-1">
            <span className="text-[10px] font-bold text-[var(--text-muted)] uppercase flex items-center gap-1.5">
              <Coins className="w-3.5 h-3.5 text-amber-500" /> Auto-Sweep Overnight FD
            </span>
            <div className="text-lg font-mono font-black text-[var(--text-primary)]">
              {formatCompactCurrency(wallet.autoSweepBalance, currency)}
            </div>
            <div className="text-[10px] font-semibold text-emerald-600 dark:text-emerald-400">
              7.1% p.a. Liquid Yield (Daily Credit)
            </div>
          </div>
          <span className="text-[10px] font-bold px-2 py-1 rounded bg-amber-500/15 text-amber-600 dark:text-amber-400 border border-amber-500/20">
            Auto-Yield
          </span>
        </div>
        ...
        <div className="glass-card p-4 flex items-center justify-between">
          <div className="space-y-1">
            <span className="text-[10px] font-bold text-[var(--text-muted)] uppercase flex items-center gap-1.5">
              <Award className="w-3.5 h-3.5 text-purple-500" /> Pledged Collateral (20% Haircut)
            </span>
            <div className="text-lg font-mono font-black text-purple-600 dark:text-purple-400">
          ...
          <span className="text-[10px] font-bold px-2 py-1 rounded bg-purple-500/15 text-purple-600 dark:text-purple-400 border border-purple-500/20">
            Pledged
          </span>
        </div>
      </div>
        ...
        <div className="glass-card p-5 lg:col-span-2 space-y-4 flex flex-col justify-between">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-[var(--text-secondary)]">Historical backtested growth of portfolio vs reference benchmark</p>
            </div>
